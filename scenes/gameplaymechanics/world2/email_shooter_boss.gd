...
@onready var email_box_notification: TextureRect = $EmailBox_Notification
...
func _on_shoot_button_pressed() -> void:
	if can_shoot:
		...
		Emails.global_position = email_spawn.global_position
		scene.add_child(Emails)
		email_box_notification.self_modulate = Color("#808080")
		can_shoot = false
		if shootlimit == 0:
			is_ranout = true
			ran_out_timer.start()
...
func _on_cooldown_timeout() -> void:
	can_shoot = true
	email_box_notification.self_modulate = Color("#ffffff")
	cooldown.stop()

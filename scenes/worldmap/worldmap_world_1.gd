...
@onready var bubble_anim_world2: AnimationPlayer = $SceneObjects/WorldMap_LevelSpots/WorldSign/SignSprite/BubbleAnim
...
@onready var unlock_world_2: Node2D = $UnlockWorld2
@onready var unlock_anim: AnimationPlayer = $UnlockWorld2/UnlockAnim
@onready var world_2_button: TextureButton = $UI/MapMenu/MapMenuPanel/MapMenuBG/HBoxContainer/World2/World2Button
@onready var world_2_text: Label = $UI/MapMenu/MapMenuPanel/MapMenuBG/HBoxContainer/World2/WorldText
...
@onready var world_sign: Node2D = $SceneObjects/WorldMap_LevelSpots/WorldSign

@onready var map_icon: TextureButton = $UI/GUI/MapIcon
...
func _ready() -> void:
	...
	if ModeManager.is_savefile1:
		...
		if SaveFileManager.world1_boss_dic["unlocked"]:
			...
			if SaveFileManager.world1_boss_dic["hasbeencompleted"]:
				...
				$SceneObjects/WorldMap_LevelSpots/ConnectGroundEnd.play("avaliable")
			else:
				level_spot_sprite_boss.play("available")
			level_spot_sprite_lv2.play("clear")
			unlock_wall_boss.disabled = true
		
		if SaveFileManager.world1_boss_dic["hasbeencompleted"] && !SaveFileManager.world_unlocks_dic["recentlyunlocked_world2"]:
			unlock_world_2.show()
			unlock_anim.play("message_popin")
			$SceneObjects/ShimejiCharacter_Playable.is_incutscene = false
			if ModeManager.is_multiplayer:
				if ModeManager.multi_2players || ModeManager.multi_3players || ModeManager.multi_4players:
					$SceneObjects/ShimejiCharacter_Playable_Player2.is_incutscene = false
				if ModeManager.multi_3players || ModeManager.multi_4players:
					$SceneObjects/ShimejiCharacter_Playable_Player3.is_incutscene = false
				if ModeManager.multi_4players:
					$SceneObjects/ShimejiCharacter_Playable_Player4.is_incutscene = false
		
		if SaveFileManager.world_unlocks_dic["world2unlocked"]:
	...
	match LanguageManager.language:
		"spanish":
			...
			$UI/GUI/CurrentWorld/CW_BG/WorldName.text = "Mundo 1"
			$UnlockWorld2/UnlockWorldThing/MessageBG/MessageText.text = "¡Felicidades! ¡Has derrotado a The Monstequare y completado el Mundo 1! 
			Ahora puedes avanzar al Mundo 2 caminando hacia adelante o utilizando el menú del mapa."
		"french":
			...
			$UI/GUI/CurrentWorld/CW_BG/WorldName.text = "Monde 1"
			$UnlockWorld2/UnlockWorldThing/MessageBG/MessageText.text = "Félicitations ! Vous avez vaincu Le Monstequare et terminé le Monde 1 ! 
			Vous pouvez maintenant passer au Monde 2 en avançant ou en utilisant le menu de la carte."
		"italian":
			...
			$UI/GUI/CurrentWorld/CW_BG/WorldName.text = "Mondo 1"
			$UnlockWorld2/UnlockWorldThing/MessageBG/MessageText.text = "Congratulazioni! Hai sconfitto The Monstequare e completato il Mondo 1!
Ora puoi passare al Mondo 2 camminando avanti o utilizzando il menu della mappa."
		"german":
			...
			$UI/GUI/CurrentWorld/CW_BG/WorldName.text = "Welt 1"
			$UnlockWorld2/UnlockWorldThing/MessageBG/MessageText.text = "Herzlichen Glückwunsch! Du hast den Monstequare besiegt und Welt 1 abgeschlossen!
Du kannst jetzt zu Welt 2 weitergehen, indem du entweder vorausläufst oder das Kartenmenü benutzt."
		"japanese":
			...
			$UI/GUI/CurrentWorld/CW_BG/WorldName.text = "ワールド1"
			$UnlockWorld2/UnlockWorldThing/MessageBG/MessageText.text = "おめでとうございます！モンスティクエアを倒してワールド1をクリアしました！
これからは、前に進むかマップメニューを使ってワールド2に進むことができます。"
		_:
			return
			#It's just English
...
func _input(event: InputEvent) -> void:
	if Input.is_action_pressed("grab"):
		...
		if on_tutorial:
			...
			if SaveFileManager.world1_tutorial_dic["trianglecollected1"]:
				triangle_counter_1.play("found")
			else:
				triangle_counter_1.play("empty")
			if SaveFileManager.world1_tutorial_dic["trianglecollected2"]:
				triangle_counter_2.play("found")
			else:
				triangle_counter_2.play("empty")
			if SaveFileManager.world1_tutorial_dic["trianglecollected3"]:
				triangle_counter_3.play("found")
			else:
				triangle_counter_3.play("empty")
			if SaveFileManager.world1_tutorial_dic["trianglecollected4"]:
		...
		if on_level1:
			...
			is_inlevelinfo = true
			$UI/LevelInfo/LevelInfoPanel/SecretUIBase.show()
			if SaveFileManager.world1_level1_dic["trianglecollected1"]:
				triangle_counter_1.play("found")
			else:
				triangle_counter_1.play("empty")
			if SaveFileManager.world1_level1_dic["trianglecollected2"]:
				triangle_counter_2.play("found")
			else:
				triangle_counter_2.play("empty")
			if SaveFileManager.world1_level1_dic["trianglecollected3"]:
				triangle_counter_3.play("found")
			else:
				triangle_counter_3.play("empty")
			if SaveFileManager.world1_level1_dic["trianglecollected4"]:
				triangle_counter_4.play("found")
			else:
				triangle_counter_4.play("empty")
		if on_level2:
			...
			is_inlevelinfo = true
			$UI/LevelInfo/LevelInfoPanel/SecretUIBase.show()
			if SaveFileManager.world1_level2_dic["trianglecollected1"]:
				triangle_counter_1.play("found")
			else:
				triangle_counter_1.play("empty")
			if SaveFileManager.world1_level2_dic["trianglecollected2"]:
				triangle_counter_2.play("found")
			else:
				triangle_counter_2.play("empty")
			if SaveFileManager.world1_level2_dic["trianglecollected3"]:
				triangle_counter_3.play("found")
			else:
				triangle_counter_3.play("empty")
			if SaveFileManager.world1_level2_dic["trianglecollected4"]:
				triangle_counter_4.play("found")
			else:
				triangle_counter_4.play("empty")
		if on_boss:
		...
		if on_world2:
			...
			go.play()
			ui.hide()
			LoadManager.load_scene("res://scenes/worldmap/worldmap_world2.tscn")
...
func _on_ok_button_pressed() -> void:
	unlock_anim.play("message_popout")
	SaveFileManager.world_unlocks_dic["recentlyunlocked_world2"] = true
	SaveGame.save_game()
...
func _on_trigger_spot_world_2_body_entered(body: Node2D) -> void:
	if (body.name == "ShimejiCharacter_Playable"):
		bubble_anim_world2.play("popin")
		bubble_pop_in.play()
		on_world2 = true


func _on_trigger_spot_world_2_body_exited(body: Node2D) -> void:
	if (body.name == "ShimejiCharacter_Playable"):
		bubble_anim_world2.play("popout")
		on_world2 = false
